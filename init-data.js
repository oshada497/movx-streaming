...
// This script adds dummy movies and TV shows with TMDB images for testing

(function initDummyContent() {
    // Check if we already have content
    const existingMovies = Storage.getMovies();
    const existingShows = Storage.getTVShows();

    if (existingMovies.length > 0 || existingShows.length > 0) {
        console.log('Content already exists. Skipping dummy data initialization.');
        return;
    }

    console.log('Initializing dummy content...');
    ...
    // Save to storage
    Storage.saveMovies(dummyMovies);
    Storage.saveTVShows(dummyTVShows);

    console.log(`Added ${dummyMovies.length} movies and ${dummyTVShows.length} TV shows`);

    // Reload content if app is initialized
